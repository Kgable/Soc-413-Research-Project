...
gss <- read_fwf(here("data","data_raw","GSS.dat"), 
                col_positions = 
                  fwf_positions(start=c(1, 6, 11, 16, 31, 35, 40, 45, 49, 54, 59, 64, 69, 74),
                                end  =c(5, 10, 15, 20, 34, 39, 44, 48, 53, 58, 63, 68, 73, 78),
...
gss <- gss |>
  mutate(race=factor(
    case_when(
      ...
      racecen1==3 | (racecen1>=11 & racecen1<=14) ~ "aian/pi",
      TRUE ~ NA_character_),
    levels=c("white","black","latinx","asian","aian/pi")))
...
gss <- gss |>
  select(year, resp_sex, educ_attain, race,
         relig_affil, religiosity, relig_attend, pray_freq, believe_god, 
         relig_imp_cat) |>
